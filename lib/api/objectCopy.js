        ...
        function goGetData(storeMetadataParams, dataLocator, destBucketMD,
            ...
            return async.mapLimit(dataLocator, 1,
                ...
                function copyPart(part, cb) {
                    ...
                    return data.get(part, passThrough, log, (err, stream) => {
                        if (err) {
                            return cb(err);
                        }
                        // if backend is Azure, get does not return stream
                        const objStream = stream || passThrough;
                        if (serverSideEncryption) {
                            ...
                            log, (err, cipherBundle) => {
                                ...
                                return data.put(cipherBundle, objStream,
                        ...
                        return data.put(null, objStream, part.size,
                        dataStoreContext, backendInfo,
